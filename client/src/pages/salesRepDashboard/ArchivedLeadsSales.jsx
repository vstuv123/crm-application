...
import { Visibility, Unarchive, Delete } from "@mui/icons-material";
import { useMemo, useRef, useState } from "react";
import { useNavigate } from 'react-router-dom';
import { Autocomplete, Box, Button, Paper, TextField, Tooltip, Typography } from "@mui/material";
import { useDeleteLeadSalesRepMutation, useGetArchivedLeadsSalesRepQuery, useUnarchiveLeadSalesRepMutation } from "../../store/slices/leadSlice";
...
const ArchivedLeadsSales = () => {
  const searchRef = useRef("");
  const [search, setSearch] = useState("");
  const [sort, setSort] = useState("");
  const [source, setSource] = useState("");
  const { data, isLoading } = useGetArchivedLeadsSalesRepQuery({ search, sort, source });
  const [unArchiveLeadSalesRep] = useUnarchiveLeadSalesRepMutation();
  const [deleteLeadSalesRep] = useDeleteLeadSalesRepMutation();
  ...
  const UnArchiveUser = async (id) => {
    try {
        toast.loading("UnArchiving...", { id: "unArchiveLead" });
        const response = await unArchiveLeadSalesRep(id).unwrap();
        toast.success(response?.message, { id: "unArchiveLead" })
    } catch (error) {
        toast.error(error.data.message, { id: "unArchiveLead" })
    }
  }

  const DeleteUser = async (id) => {
    try {
        toast.loading("Deleting...", { id: "deleteLead" });
        const response = await deleteLeadSalesRep(id).unwrap();
        toast.success(response?.message, { id: "deleteLead" })
    } catch (error) {
        toast.error(error.data.message, { id: "deleteLead" })
    }
  }
    ...
    },
    {
      ...
      Cell: ({ row }) => (
        <div>
            ...
            <Tooltip title="Delete Lead Permanently">
              <Delete
    ...
    <>
        {
            (isLoading || !data) ? (
              ...
              <Box className="container">
                <MetaData title={"CRM - Archived Leads Sales Rep"} />
                <Box sx={{ mt: 1, mb: 1 }}>
                    <Typography sx={{ textAlign: "center", color: "red", fontFamily: "Roboto", fontSize: "18px" }}><strong>Note:</strong>&nbsp; Archived Leads will automatically be deleted after 30 days</Typography>
                </Box>
...
export default ArchivedLeadsSales