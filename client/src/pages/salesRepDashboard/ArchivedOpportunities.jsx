...
import {
  ...
  useGetArchivedOpportunitiesQuery,
  useUnarchiveOpportunityMutation,
...
const ArchivedOpportunities = () => {
  ...
  const { data, isLoading } = useGetArchivedOpportunitiesQuery({
    search,
    sort,
    stage,
    closedDate,
  });
  const [unArchiveOpportunity] = useUnarchiveOpportunityMutation();
  ...
  const UnArchiveUser = async (id) => {
    try {
      toast.loading("UnArchiving...", { id: "unArchiveOpportunity" });
      const response = await unArchiveOpportunity(id).unwrap();
      toast.success(response?.message, { id: "unArchiveOpportunity" });
    } catch (error) {
      toast.error(error.data.message, { id: "unArchiveOpportunity" });
    }
  };

  const DeleteUser = async (id) => {
    try {
      toast.loading("Deleting...", { id: "deleteOpportunity" });
      const response = await deleteOpportunity(id).unwrap();
      toast.success(response?.message, { id: "deleteOpportunity" });
    } catch (error) {
      toast.error(error.data.message, { id: "deleteOpportunity" });
    }
  };
      ...
      },
      {
        ...
        Cell: ({ row }) => (
          <div style={{ display: "flex", alignItems: "center" }}>
            ...
            <Tooltip title="UnArchive Opportunity">
              <Unarchive
                style={{ cursor: "pointer", marginRight: "10px" }}
                //eslint-disable-next-line
            ...
            <Tooltip title="Delete Opportunity Permanently">
              <Delete
...
export default ArchivedOpportunities;
