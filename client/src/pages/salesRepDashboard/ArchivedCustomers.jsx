...
import { Visibility, Delete, Unarchive } from "@mui/icons-material";
import { useMemo, useRef, useState } from "react";
import { useNavigate } from 'react-router-dom';
import { Autocomplete, Box, Button, Paper, TextField, Tooltip, Typography } from "@mui/material";
import MetaData from "../../components/layout/MetaData";
import { useDeleteCustomerSalesRepMutation, useGetArchivedCustomersSalesQuery, useUnarchiveCustomerSalesRepMutation } from "../../store/slices/customerSlice";
...
const ArchivedCustomers = () => {
  ...
  const { data, isLoading } = useGetArchivedCustomersSalesQuery({ search, sort, company, industry });
  const [unArchiveCustomerSalesRep] = useUnarchiveCustomerSalesRepMutation();
  const [deleteCustomerSalesRep] = useDeleteCustomerSalesRepMutation();
  ...
  const UnArchiveUser = async (id) => {
    try {
        toast.loading("UnArchiving...", { id: "unArchiveCustomer" });
        const response = await unArchiveCustomerSalesRep(id).unwrap();
        toast.success(response?.message, { id: "unArchiveCustomer" })
    } catch (error) {
        toast.error(error.data.message, { id: "unArchiveCustomer" })
    }
  }

  const DeleteUser = async (id) => {
    try {
        toast.loading("Deleting...", { id: "deleteCustomer" });
        const response = await deleteCustomerSalesRep(id).unwrap();
        toast.success(response?.message, { id: "deleteCustomer" })
    } catch (error) {
        toast.error(error.data.message, { id: "deleteCustomer" })
    }
  }
    ...
    },
    {
      ...
      Cell: ({ row }) => (
        <div style={{ display: "flex", alignItems: "center" }}>
            ...
            <Tooltip title="UnArchive Customer">
              <Unarchive
              style={{ cursor: "pointer", marginRight: "10px" }}
              //eslint-disable-next-line
              onClick={() => UnArchiveUser(row.original.id)}
              />
            </Tooltip>
            <Tooltip title="Delete Customer Permanently">
              <Delete
    ...
    <>
        {
            (isLoading || !data) ? (
              ...
              <Box className="container">
                <MetaData title={"CRM - Archived Customers SalesRep"} />
                <Box sx={{ mt: 1, mb: 1 }}>
                    <Typography sx={{ textAlign: "center", color: "red", fontFamily: "Roboto", fontSize: "18px" }}><strong>Note:</strong>&nbsp; Archived Customers will automatically be deleted after 30 days</Typography>
                </Box>
