...
import { useArchiveCustomerSalesRepMutation, useGetNonArchivedCustomersSalesQuery } from "../../store/slices/customerSlice";
...
const UnArchivedCustomers = () => {
  const searchRef = useRef("");
  const [search, setSearch] = useState("");
  const [sort, setSort] = useState("");
  const [company, setCompany] = useState("");
  const [industry, setIndustry] = useState("");
  const { data, isLoading } = useGetNonArchivedCustomersSalesQuery({ search, sort, company, industry });
  const [archiveCustomerSalesRep] = useArchiveCustomerSalesRepMutation();
  ...
  const archiveCustomer = async (id) => {
    try {
        toast.loading("Archiving...", { id: "archiveCustomer" });
        const response = await archiveCustomerSalesRep(id).unwrap();
        toast.success(response?.message, { id: "archiveCustomer" })
    } catch (error) {
        toast.error(error.data.message, { id: "archiveCustomer" })
    }
  }
    ...
    <>
        {
            (isLoading || !data) ? (
              ...
              <Box className="container">
                <MetaData title={"CRM - UnArchived Customers SalesRep"} />
...
export default UnArchivedCustomers