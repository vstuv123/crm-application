...
import { useGetOpportunitiesOfLeadMutation } from "../../store/slices/opportunitySlice";

const OpportunitiesOfSingleLead = () => {
  const location = useLocation();
  const [id, setId] = useState(location.state?.id || "");
  const [getOpportunitiesOfLead, { isLoading }] =
    useGetOpportunitiesOfLeadMutation();
  ...
  const handleSearch = async () => {
    const IdData = { id };
    try {
      const response = await getOpportunitiesOfLead(IdData).unwrap();
      setData(response?.opportunities);

      sessionStorage.setItem("id", id);
      sessionStorage.setItem("data", JSON.stringify(response?.opportunities));
      ...
      navigate(".", {
        ...
        state: {
          ...
          from: "/sales/dashboard/opportunities/lead",
        },
      });
    } catch (error) {
  ...
  const displayedLeads = useMemo(() => {
    ...
    return data.map((opportunity) => ({
      ...
      archived: opportunity.isArchived === true ? "Archived" : "NonArchived",
    }));
  }, [data]);
    ...
    <>
      {isLoading || !data ? (
        ...
        <Box className="container">
          <MetaData
            title={"CRM - Search Opportunities Of Single Lead Sales Rep"}
          />
          <Box
            sx={{
              display: "flex",
...
export default OpportunitiesOfSingleLead;
