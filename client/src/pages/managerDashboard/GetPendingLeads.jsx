...
import { useGetPendingLeadsQuery, useUpdatePendingLeadStatusManagerMutation } from "../../store/slices/leadSlice";
...
const GetPendingLeads = () => {
    ...
    const { data, isLoading } = useGetPendingLeadsQuery({ search, sort, source });
    const [updatePendingLeadStatusManager] = useUpdatePendingLeadStatusManagerMutation();
    ...
    const handleUpdatePendingStatusApproved = async (id) => {
        const leadData = { id, status: "New" }
        try {
            toast.loading("Updating...", {id: "pending"});
            const response = await updatePendingLeadStatusManager(leadData).unwrap();
            toast.success(response?.message, {id: "pending"});
        } catch (error) {
            toast.error(error.data.message, {id: "pending"});
        }
    }
      ...
      <>
          {
              (isLoading || !data) ? (
                ...
                <Box className="container">
                  <MetaData title={"CRM - Pending Leads Manager"} />
                  <Box sx={{ mt: 1, mb: 1 }}>
                      <Typography sx={{ textAlign: "center", color: "red", fontFamily: "Roboto", fontSize: "18px" }}><strong>Note:</strong>&nbsp; Rejected Leads will automatically be archived and hence deleted after 30 days while Approved Leads will be un-archived and their status will automatically be changed to New.</Typography>
                  </Box>
...
export default GetPendingLeads
