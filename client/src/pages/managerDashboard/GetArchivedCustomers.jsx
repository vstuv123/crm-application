...
import { useGetArchivedCustomersQuery } from "../../store/slices/customerSlice";
...
const GetArchivedCustomers = () => {
  const searchRef = useRef("");
  const [search, setSearch] = useState("");
  const [sort, setSort] = useState("");
  const [company, setCompany] = useState("");
  const [industry, setIndustry] = useState("");
  const { data, isLoading } = useGetArchivedCustomersQuery({ search, sort, company, industry });
  ...
  const handleEditUser = (id) => {
    navigate(`/manager/dashboard/update-customer/${id}`);
  }
    ...
    <>
        {
            (isLoading || !data) ? (
              ...
              <Box className="container">
                <MetaData title={"CRM - Archived Customers Manager"} />
                <Box sx={{ mt: 1, mb: 1 }}>
...
export default GetArchivedCustomers