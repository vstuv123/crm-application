...
import { useSalesOfSaleRepMutation } from "../../store/slices/saleSlice";

const GetSalesOfSalesRep = () => {
  const location = useLocation();
  const [id, setId] = useState(location.state?.id || "");
  const [getSalesOfSalesRep, { isLoading }] = useSalesOfSaleRepMutation();
  ...
  const handleSearch = async () => {
    const IdData = { id };
    try {
      const response = await getSalesOfSalesRep(IdData).unwrap();
      ...
      navigate(".", {
        ...
        state: {
          id,
          data: response?.sales,
          from: "/manager/dashboard/sales/sales-rep",
        },
      });
    } catch (error) {
    ...
    <>
      {isLoading || !data ? (
        ...
        <Box className="container">
          <MetaData title={"CRM - Search Sales By Sales Rep Id Manager"} />
          <Box
            sx={{
              display: "flex",
...
export default GetSalesOfSalesRep;
