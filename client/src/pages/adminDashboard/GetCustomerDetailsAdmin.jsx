...
import { useGetCustomerDetailsAdminQuery } from '../../store/slices/customerSlice';

const GetCustomerDetailsAdmin = () => {
    const { id } = useParams();
    const navigate = useNavigate();
    const {data, isLoading} = useGetCustomerDetailsAdminQuery(id);
    ...
    const handleUpdateCustomer = () => {
        if (data?.customer?.isArchived === true) {
            navigate(`/admin/dashboard/customers/archived`);
        }else {
            navigate(`/admin/dashboard/customers/un-archived`);
        }
    }
  return (
    <Box sx={{ height: "100%", width: "100%", display: "flex", justifyContent: "center", alignItems: "center", gap: "100px" }} >
      <MetaData title={"CRM - Customer Details Admin"} />
      ...
      <Box sx={{ display: "flex", justifyContent: "center", alignItems: "center" }}>
        <Box sx={{ backgroundColor: "#fff", height: "100%", width: {md: "420px", sm: "400px", xs: "370px"}, mt: { md: 15, sm: 25, xs: 25} }}>
            <Box sx={{ display: "flex", flexDirection: "column", gap: "25px" }}>
                ...
                <Button onClick={handleUpdateCustomer} sx={{ ml: 5, mb: 3, width: "80%", background: "#d39925", color: "white", '&:hover': {backgroundColor: "#c08717"}, '&:focus': {backgroundColor: "#c08717"}, '&:active': {backgroundColor: "#c08717"} }} >Go Back</Button>
            </Box>
        </Box>
      </Box>
    </Box>
...
export default GetCustomerDetailsAdmin