...
import { useGetArchivedCustomersAdminQuery } from "../../store/slices/customerSlice";
...
const GetArchivedCustomersAdmin = () => {
  const searchRef = useRef("");
  const [search, setSearch] = useState("");
  const [sort, setSort] = useState("");
  const [company, setCompany] = useState("");
  const [industry, setIndustry] = useState("");
  const { data, isLoading } = useGetArchivedCustomersAdminQuery({ search, sort, company, industry });
  const navigate = useNavigate();
  const handleGetUserDetails = (id) => {
    navigate(`/admin/dashboard/customer-details/${id}`);
  }
    ...
    <>
        {
            (isLoading || !data) ? (
              ...
              <Box className="container">
                <MetaData title={"CRM - Archived Customers Admin"} />
                <Box sx={{ mt: 1, mb: 1 }}>
                ...
                <Box className="span">
                    {
                      (search || company || industry) ? (
                        <span>{displayedCustomers?.length} Documents matches your search</span>
                      ) : (
                        <span style={{ marginLeft: "50px" }}>Total {displayedCustomers?.length} Documents</span>
                      )
                    }
                </Box>
...
export default GetArchivedCustomersAdmin